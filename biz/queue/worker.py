...
from biz.platforms.yunxiao.webhook_handler import MergeRequestHandler as YunxiaoMergeRequestHandler, \
    PushHandler as YunxiaoPushHandler
...
def handle_yunxiao_push_event(webhook_data: dict, yunxiao_token: str, yunxiao_url: str, yunxiao_url_slug: str):
    push_review_enabled = os.environ.get('PUSH_REVIEW_ENABLED', '0') == '1'
    try:
        handler = YunxiaoPushHandler(webhook_data, yunxiao_token, yunxiao_url)
        logger.info('Yunxiao Push Hook event received')
        commits = handler.get_push_commits()
        if not commits:
            logger.error('Failed to get commits')
            return
...
def handle_yunxiao_merge_request_event(webhook_data: dict, yunxiao_token: str, yunxiao_url: str, yunxiao_url_slug: str):
    '''
    处理Yunxiao Merge Request Hook事件
    '''
    merge_review_only_protected_branches = os.environ.get('MERGE_REVIEW_ONLY_PROTECTED_BRANCHES_ENABLED', '0') == '1'
    try:
        # 解析Webhook数据
        handler = YunxiaoMergeRequestHandler(webhook_data, yunxiao_token, yunxiao_url)
        logger.info('Yunxiao Merge Request Hook event received')
        ...
        event_manager['merge_request_reviewed'].send(
            MergeRequestReviewEntity(
                ...
                source_branch=webhook_data['object_attributes']['source_branch'],
                target_branch=webhook_data['object_attributes']['target_branch'],
                updated_at=int(datetime.now().timestamp()), # Yunxiao timestamps might need parsing if not int
                commits=commits,
                score=CodeReviewer.parse_review_score(review_text=review_result),
                url=webhook_data['object_attributes']['url'],
                review_result=review_result,
